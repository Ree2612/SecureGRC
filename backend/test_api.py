...
def test_full_api():
    with TestClient(app) as client:
        ...
        print("\nTesting /api/v1/logs (Database Logs)...")
        logs_resp = client.get("/api/v1/logs", headers=headers)
        assert logs_resp.status_code == 200, f"Logs failed: {logs_resp.text}"
        logs = logs_resp.json()
        print(f"Retrieved {len(logs)} database logs.")

        print("\nTesting /api/v1/logs/stats...")
        stats_resp = client.get("/api/v1/logs/stats", headers=headers)
        assert stats_resp.status_code == 200
        print("Log stats:", stats_resp.json())
